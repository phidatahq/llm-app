...
```bash
docker exec -it llm-dev-api alembic -c db/alembic.ini revision --autogenerate -m "Initialize DB"
```
...
```bash
docker exec -it llm-dev-api alembic -c db/alembic.ini upgrade head
```
